...
import type { LocalOperation, RowValue, ServerChange } from "../src";
import { compareOperations, rebaseAndReplay } from "../src/internal";

const op = (over: Partial<LocalOperation> & Pick<LocalOperation, "opId" | "kind">): LocalOperation => ({
  clientId: "c",
  userId: "u",
  schemaVersion: 1,
  functionName: "fn",
...
describe("rebaseAndReplay", () => {
  it("applies server changes then replays pending local ops on top", () => {
    const canonical: RowValue[] = [{ _id: "t1", _table: "todos", text: "orig", title: "a", done: false, _version: 1 }];
    ...
    const { rows, conflicts } = rebaseAndReplay({ canonicalRows: canonical, serverChanges, pendingOperations: pending });
  ...
  it("reports a conflict when a pending patch targets a missing row", () => {
    ...
    const { conflicts } = rebaseAndReplay({ canonicalRows: [], serverChanges: [], pendingOperations: pending });
...
describe("compareOperations", () => {
  it("orders by createdAt then opId, deterministically across shuffles", () => {
    ...
    const order1 = [c, a, b].slice().sort(compareOperations).map((entry) => entry.opId);
    const order2 = [b, c, a].slice().sort(compareOperations).map((entry) => entry.opId);
