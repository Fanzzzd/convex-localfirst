...
import { LocalCache, LocalFirstEngine, SearchIndex, SearchManager, looksLikeHtml, stripHtml, tokenize } from "../../src/core/internal";
...
function idx(fields: string[], rows: Array<{ id: string; values: unknown[]; recency?: number | null }>): SearchIndex {
...
describe("SearchIndex ranking", () => {
  ...
  it("reports total and honors limit", () => {
    const index = idx(["title"], Array.from({ length: 10 }, (_, i) => ({ id: `r${i}`, values: ["match"] })));
...
function change(id: string, value: Record<string, unknown>, kind: ServerChange["kind"] = "insert", version = 1, ws = "w1"): ServerChange {
  const base = { changeId: `c-${id}-${version}`, scopeKey: `byWorkspace:${ws}`, table: "issues", id, kind, version, serverTime: version };
...
async function seededManager(seed: ServerChange[]): Promise<{ cache: LocalCache; manager: SearchManager }> {
  const store = new MemoryLocalStore();
  for (const c of seed) await store.applyServerChange(c);
  const manifest = searchManifest();
  const host = new LocalFirstEngine({ manifest, store: new MemoryLocalStore(), clientId: "c", userId: "u", nameOf: (r) => String(r) });
...
describe("SearchManager (delta-maintained, no rescans)", () => {
  it("builds from the hydrated cache and resolves live rows; HTML fields are searchable", async () => {
    const { manager } = await seededManager([
      change("i1", { workspaceId: "w1", title: "Login bug", description_html: "<p>Cannot <b>sign in</b></p>" }),
  ...
  it("updates a live search from deltas (insert/patch/delete) without a rebuild", async () => {
    const { cache, manager } = await seededManager([change("i1", { workspaceId: "w1", title: "alpha", description_html: "" })]);
    let notes = 0;
    ...
    cache.applyServerChanges([change("i2", { workspaceId: "w1", title: "alpha two", description_html: "" })]);
    expect(notes).toBeGreaterThan(0);
    expect(new Set(sub.current().results.map((r) => r._id))).toEqual(new Set(["i1", "i2"]));

    // PATCH i1's title away from "alpha" → it leaves the result.
    cache.applyServerChanges([change("i1", { workspaceId: "w1", title: "renamed", description_html: "" }, "patch", 2)]);
  ...
  it("filters by scope and reflects it in total", async () => {
    ...
    const scoped = manager.subscribe("issues", "shared", { scope: { workspaceId: "w1" } }, () => {});
  ...
  it("yields nothing for a table without searchFields", async () => {
    const { manager } = await seededManager([change("i1", { workspaceId: "w1", title: "x", description_html: "" })]);
  ...
  it("keeps a stable result reference when an unrelated change lands", async () => {
    ...
    cache.applyServerChanges([change("i2", { workspaceId: "w1", title: "beta", description_html: "" })]);
...
describe("SearchManager (standalone over a minimal cache)", () => {
  it("builds once and maintains from deltas", () => {
    ...
    const manifest = defineLocalFirstManifest({
      schemaVersion: 1,
      tables: { docs: localTable({ table: "docs", idField: "localId", scope: byWorkspace({ workspaceIdField: "w", membershipTable: "m" }), indexes: {}, searchFields: ["title"] }) },
      queries: {},
      mutations: {}
    });
    const manager = new SearchManager(cache, manifest);
    ...
    for (const l of listeners) l([{ table: "docs", localId: "d2", kind: "upsert", row: rows.get("d2")! }]);
