...
function change(table: string, id: string, value: Record<string, unknown>, version = 1): ServerChange {
  return {
...
describe("createLocalDb", () => {
  ...
  it("returns the same rows and query plan as equivalent collection combinators", async () => {
    ...
    expect(engine.explainQuery(typed)).toMatchObject({ strategy: "index", index: "byWorkspaceTitle" });
  });
