...
import { defineLocalFirstManifest } from "../../src/core/index.js";
import { createConvexLocalFirst } from "../../src/react";
...
test("createConvexLocalFirst engine: mutate/query infer args + result from the ref", () => {
  ...
  type ListTodos = FunctionReference<
    "query",
    "public",
    { listId: string },
  ...
  expectTypeOf(engine.mutate(create, { title: "x" }).server).resolves.toEqualTypeOf<{
    id: string;
  }>();
