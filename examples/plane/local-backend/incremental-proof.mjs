...
function projectInsert(localId, name) {
  ...
  return {
    ...
    clientId,
    schemaVersion: SCHEMA_VERSION,
    functionName: "projects:create",
    table: "projects",
    kind: "insert",
    localId,
    value: { workspace: WS, name, identifier: "INC", created_at: ts, updated_at: ts },
  };
}
...
assert(
  delta.changes.some((ch) => ch.localId === id),
  "delta pull from cursor returns the new row",
);
assert(
  delta.changes.length <= 3,
  `delta is small (got ${delta.changes.length}, expected ~1) — cost tracks changes, not table size`,
);
...
assert(
  ...
  `pull past the new row is empty again (got ${after.changes.length}) — cursor is monotonic`,
);
