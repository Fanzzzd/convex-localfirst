    ...
    pull: async () => {
      ...
      for (let i = 0; i < 50; i++) {
        const r = await c.query(pullRef, {
          ...
          userId: subject,
          schemaVersion: SCHEMA_VERSION,
          scopes: [{ kind: "byWorkspace", value: WS }],
          cursors,
        });
        all.push(...r.changes);
        Object.assign(cursors, r.cursors);
...
if (cmd === "write") {
  ...
  if (res.rejected?.length) {
    console.error("FAIL: write rejected", res.rejected);
    process.exit(1);
  }
  const changes = await alice.pull();
  const seen = changes.some((c) => c.localId === localId && c.table === "projects");
  if (!seen) {
    console.error("FAIL: row not pullable immediately after write");
    process.exit(1);
  }
  ...
  if (!localId) {
    console.error("usage: verify <localId>");
    process.exit(2);
  }
  ...
  const seen = changes.some((c) => c.localId === localId && c.table === "projects");
  if (!seen) {
    console.error(`FAIL: row ${localId} missing after crash+restart (NOT durable)`);
    process.exit(1);
  }
