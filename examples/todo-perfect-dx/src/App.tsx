...
export function App() {
  ...
  const labels =
    useLiveQuery(
      collection<Doc<"labels">>("labels").scope({ workspaceId: WORKSPACE }).order("name"),
    ) ?? [];
  ...
  const move = (issue: Issue, delta: number) => {
    const next =
      ORDER[Math.max(0, Math.min(ORDER.length - 1, ORDER.indexOf(issue.status) + delta))];
          ...
          >
            <span
              className={cn(
                ...
                sync.online ? "text-emerald-600" : "text-red-600",
              )}
            >
            ...
            {sync.blockedBySchemaMismatch ? (
              <Badge variant="destructive">schema mismatch</Badge>
            ) : null}
          </div>
        </header>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mb-3 flex items-center gap-2">
                              ...
                              >
                                <Badge className={cn("cursor-pointer", pr.className)}>
                                  {pr.label}
                                </Badge>
                              </button>
                            ...
                            <div className="flex items-center gap-1 opacity-60 transition-opacity group-hover:opacity-100">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const next = labels.find(
                                    (l) => !issueLabels.some((il) => il._id === l._id),
                                  );
                                  if (next)
                                    void linkLabel({
                                      workspaceId: WORKSPACE,
